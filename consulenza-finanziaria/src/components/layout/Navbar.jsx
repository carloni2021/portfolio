      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          >
            ...
            <span className={`font-semibold text-lg tracking-tight transition-colors ${scrolled ? 'text-gray-900' : 'text-gray-900'}`}>
              {config.brand}
            </span>
          </a>
          ...
          <div className="hidden md:flex items-center gap-8">
            {config.navLinks.map((link) => (
              <a
                ...
                className="text-gray-600 hover:text-forest-900 text-sm font-medium transition-colors duration-200"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex flex-col gap-1 pt-2 border-t border-gray-100">
            {config.navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={(e) => { e.preventDefault(); handleNavClick(link.href) }}
                className="text-gray-700 hover:text-forest-900 text-sm font-medium py-3 px-2 rounded-lg hover:bg-gray-50 transition-all duration-200"
              >
                {link.label}
              </a>
            ))}
